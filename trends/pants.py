...
def extract_sixth_texture_image(gltf):
    if len(gltf.textures) > 8:
        image_index = gltf.textures[8].source  # 6th texture (index 5)
        ...
        path = os.path.join(EXTRACTED_IMAGES_DIR, 'ninth_texture.png')
        with open(path, 'wb') as f:
            f.write(image_data)
        return path
    else:
        raise Exception("Less than 9 textures in the GLB file")
...
def process_glb_and_compare_pants(glb_url, dataset_csv):
